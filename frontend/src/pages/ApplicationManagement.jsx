...
import { ArrowLeft, Calendar, FileSpreadsheet, UploadCloud, CheckCircle, AlertTriangle, Download, List, FileCheck, Info, Loader2, BarChart2 } from 'lucide-react';
...
export const ApplicationManagement = () => {
  ...
  useEffect(() => {
    ...
    return () => { unsubActivities(); unsubMembers(); unsubMachines(); unsubSettings(); };
  }, []);
  ...
  const paymentOptions = useMemo(() => {
    ...
    activities.forEach(a => {
        if (a.paymentDateId === 'custom' && a.customPaymentDate) {
  ...
  const annualSummary = useMemo(() => {
    ...
    activities.forEach(act => {
      ...
      if (act.paymentDateId === 'custom' && act.customPaymentDate) {
        ...
        if (periods.some(p => p.id === act.paymentDateId)) {
          periodKey = act.paymentDateId;
        }
      }
    ...
    // カスタムと未設定は、0円なら列ごと非表示にする
    const visiblePeriods = periods.filter(p => 
      (!p.id.startsWith('custom_') && p.id !== 'unassigned_period') || p.total > 0
    );
  ...
  const handleDownloadDiff = async () => {
    ...
    mismatches.forEach(m => {
        if (m.rowIdx && m.colIdx) {
            sheet.cell(m.rowIdx, m.colIdx).style("fill", "ffcccc"); 
        }
        if (m.rowIdx2 && m.colIdx) {
            sheet.cell(m.rowIdx2, m.colIdx).style("fill", "ffcccc");
        }
        if ((m.type === '対象外活動' || m.type === '活動未登録') && m.colIdx) {
            sheet.cell(10, m.colIdx).style("fill", "ffcccc");
        }
    });
                ...
                <table className="w-full text-left text-sm whitespace-nowrap">
                  ...
                  <tbody>
                    {filteredActivities.length === 0 ? (
                        <tr><td colSpan="8" className="p-6 text-center text-gray-400 font-bold">この申請時期に該当する活動はありません</td></tr>
                    ) : (
                        <>
                            {filteredActivities.map((act, index) => {
                                const cost = calculateCost(act);
                                return (
                                    <tr key={act.id} className="border-b border-gray-50 hover:bg-green-50 transition-colors cursor-pointer" onClick={() => navigate(`/activity-form/${act.id}`, { state: { isViewMode: true }})}>
                            ...
                            {categoryTotals.map((cat, idx) => (
                                <tr key={`cat-${idx}`} className="bg-gray-50 border-t border-gray-200">
                                    <td className="p-3 text-right font-bold text-gray-600" colSpan="5">【小計】 {cat.name}</td>
                            ...
                            <tr className="bg-blue-50/50 border-t-2 border-blue-200 font-bold">
                                <td className="p-3 text-center text-blue-800" colSpan="5">総合計</td>
