...
import { Calendar, CheckCircle, Plus, Settings, LogOut, Sprout, Users, UserCog, MessageSquare, Trash2, X, MapPin, BarChart2, Activity, Printer, FileSpreadsheet, LayoutList, Layers } from 'lucide-react';
  ...
  const ActivityCard = ({ activity }) => {
    ...
    return (
      <div onClick={() => navigate('/activity-form', { state: { editData: activity, isViewMode: true } })} className="bg-white rounded-2xl shadow-sm border-l-4 border-green-500 p-4 cursor-pointer hover:shadow-md transition-all flex flex-col h-full relative group">
        {groupInfo ? (
          <span className="absolute top-4 right-4 bg-gray-100 text-gray-600 text-[10px] px-2 py-1 rounded-md font-bold">
            {groupInfo.name}
          </span>
        ) : (
          <span className="absolute top-4 right-4 bg-red-50 text-red-500 text-[10px] px-2 py-1 rounded-md font-bold border border-red-100">
            未登録
          </span>
        )}
        <h3 className="font-bold text-lg text-gray-900 mb-2 pr-24 leading-tight">{activity.activityType || '内容未入力'}</h3>
          ...
          <>
            ...
            {displayMode === 'group' && (
              <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                {/* 🚀 グループ未登録・削除されたグループの作業 */}
                {(() => {
                  // groupListにIDが存在しないものを全て抽出
